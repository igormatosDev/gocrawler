...
var dbinfo = fmt.Sprintf("user=%s password=%s dbname=%s sslmode=disable", "postgres", "postgres", "gocrawler")
	...
	for _, n := range htmlquery.Find(doc, "//a/@href") {
		...
		if isLinkAllowed(link) {
			if strings.Contains(link, "segurospromo") || strings.Contains(link, "passagenspromo") {
				...
				err = db.QueryRow("INSERT INTO reference(father,link,link_text) VALUES($1,$2,$3) returning id", mainLink, link, "").Scan(&lastInsertId)
